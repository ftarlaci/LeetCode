...
public:
    int findTilt(TreeNode* root) {
        return goInDepth(root).first;        
    }
private:
    std::pair<int, int> goInDepth(TreeNode* root)
    {   
        if (!root) return std::make_pair(0, 0);
        auto lft = goInDepth(root->left);
        auto rht = goInDepth(root->right);
        return make_pair(abs(lft.second - rht.second) + lft.first + rht.first, lft.second + root->val + rht.second);
    }
};